...
function OrderStatus({ordersObj}) {

    //checking if the user has the "purchased" object which contains all the products that user puchased
    const orders = Object.hasOwn(ordersObj, 'purchased') ? ordersObj.purchased : {};

    let orderStatus = <div className={styles.orderStatusMain}>
            <h4>You dont have any orders yet</h4>
        </div>

    //if user has products then display it
    if (orders.products){
